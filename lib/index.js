...
function resetData (vm, meta) {
  ...
  if (resetPath.length) {
    ...
    resetPath.forEach(function (path) {
      vm.$set(path, util.getPathValue(data, path))
    })
  }
}
...
function routeQueryWatcher (query, oldQuery) {
  let vm = this
  ...
  if (vm.__winHistoryLen__ === winHistoryLen) {
    resetDateAndParseQuery(vm, meta, query)
  } else {
    ...
    const verKey = meta.urlVersionKey
    if (query[verKey] !== oldQuery[verKey]) {
      resetDateAndParseQuery(vm, meta, query)
    }
  }
}
